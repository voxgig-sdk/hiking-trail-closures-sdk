...
//    only - a typed HikingTrailClosuresContext there would make every ctx
...
final class HikingTrailClosuresOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new HikingTrailClosuresContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new HikingTrailClosuresSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new HikingTrailClosuresResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new HikingTrailClosuresError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new HikingTrailClosuresResponse($clean['response']);
    ...
    public static function err_from_map($m): ?HikingTrailClosuresError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new HikingTrailClosuresError(is_string($code) ? $code : '', $msg);
    }
