...
// HikingTrailClosures SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = HikingTrailClosuresOmni::makeStructRunner(
    ...
    {
        return HikingTrailClosuresOmni::objmodel(HikingTrailClosuresOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return HikingTrailClosuresOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return HikingTrailClosuresOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(HikingTrailClosuresOmni::structfix($now, Struct::undef()))
                        !== json_encode(HikingTrailClosuresOmni::structfix($val, Struct::undef()))) {
