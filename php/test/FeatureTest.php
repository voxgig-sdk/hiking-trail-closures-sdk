...
// HikingTrailClosures SDK feature test
//
...
class FtCtrl extends HikingTrailClosuresControl
{
...
class FtClient
{
    ...
    private HikingTrailClosuresUtility $utility;

    public function __construct(HikingTrailClosuresUtility $utility, array $options)
    {
    ...
    public function get_utility(): HikingTrailClosuresUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (HikingTrailClosuresContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public HikingTrailClosuresContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (HikingTrailClosuresContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = HikingTrailClosuresConfig::make_feature($name);
    ...
    private static function buildUrl(HikingTrailClosuresSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof HikingTrailClosuresError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new HikingTrailClosuresResponse($fetched);
        }
    ...
    private function fail(HikingTrailClosuresContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (HikingTrailClosuresContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
