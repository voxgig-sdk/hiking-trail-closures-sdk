...
// HikingTrailClosures SDK TrailClosure entity
...
class TrailClosureEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new TrailClosureEntity($this->_client, $opts);
    }

    /**
     * @param TrailClosure|array $args TrailClosure data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = HikingTrailClosuresHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return TrailClosure|array The current TrailClosure data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of TrailClosure fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = HikingTrailClosuresHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of TrailClosure fields).
     */
    ...
    /**
     * List TrailClosure items matching the given filter.
     *
     * @param TrailClosureListMatch|array|null $reqmatch Match filter (any subset
     *   of TrailClosure fields) as an assoc-array; TrailClosureListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return TrailClosure[]|array A list of TrailClosure items as assoc-arrays at
     *   the SDK boundary; throws HikingTrailClosuresError on failure (item-5 convention).
     */
