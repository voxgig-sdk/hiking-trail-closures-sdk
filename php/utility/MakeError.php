...
class HikingTrailClosuresMakeError
{
    public static function call(?HikingTrailClosuresContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new HikingTrailClosuresContext([], null);
        }
        $op = $ctx->op ?? new HikingTrailClosuresOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new HikingTrailClosuresResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof HikingTrailClosuresError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new HikingTrailClosuresError('', $msg, $ctx);
        ...
        if ($err instanceof HikingTrailClosuresError) {
