...
// HikingTrailClosures SDK retry feature
...
class HikingTrailClosuresRetryFeature extends HikingTrailClosuresBaseFeature
{
    ...
    public function init(HikingTrailClosuresContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (HikingTrailClosuresContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(HikingTrailClosuresContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(HikingTrailClosuresContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
