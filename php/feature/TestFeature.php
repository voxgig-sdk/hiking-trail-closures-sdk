...
// HikingTrailClosures SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class HikingTrailClosuresTestFeature extends HikingTrailClosuresBaseFeature
{
    ...
    public function init(HikingTrailClosuresContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (HikingTrailClosuresContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = HikingTrailClosuresParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (HikingTrailClosuresContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(HikingTrailClosuresContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = HikingTrailClosuresParam::call($ctx, $k);
