...
Hiking Trail Closures client, generated from the OpenAPI spec.

This API provides up-to-date information about closures and diversions of hiking trails as well as cycle and mountain bike routes in Switzerland.

Learn more about Hiking Trail Closures at [data.geo.admin.ch](https://data.geo.admin.ch).

This is an unofficial SDK for the Hiking Trail Closures public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — TrailClosure — that you
...
const items = await client.TrailClosure().list()
```
...
```go
client := sdk.Test()
result, err := client.TrailClosure(nil).List(
    nil, nil,
)
```
...
| --- | --- | --- |
| TypeScript | `@voxgig-sdk/hiking-trail-closures` | publish pending — [install from git tag](https://github.com/voxgig-sdk/hiking-trail-closures-sdk/releases) |
...
```bash
cd go-mcp && go build -o hiking-trail-closures-mcp .
```
...
```json
{
  "mcpServers": {
    "hiking-trail-closures": {
      "command": "/abs/path/to/hiking-trail-closures-mcp"
    }
  }
}
```
...
| --- | --- | --- |
| **TrailClosure** | The TrailClosure entity (list). | `/ch.astra.wanderland-sperrungen_umleitungen/` |
...
# List all trailclosures (returns a list, raises on error)
trailclosures = client.TrailClosure().list()
...
// List all trailclosures (returns an array; throws on error)
$trailclosures = $client->TrailClosure()->list();
print_r($trailclosures);
```

### Golang

```go
import sdk "github.com/voxgig-sdk/hiking-trail-closures-sdk/go"

client := sdk.New()

// List all trailclosures
trailClosures, err := client.TrailClosure(nil).List(nil, nil)
if err != nil {
    panic(err)
}
fmt.Println(trailClosures)
```
...
# List all trailclosures (returns an Array; raises on error)
trailclosures = client.TrailClosure.list
puts trailclosures
```
...
Generated from the Hiking Trail Closures OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
