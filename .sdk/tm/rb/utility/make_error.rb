...
module HikingTrailClosuresUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = HikingTrailClosuresContext.new({}, nil)
    end
    op = ctx.op || HikingTrailClosuresOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || HikingTrailClosuresResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(HikingTrailClosuresError) ? err.msg : err.to_s
    ...
    sdk_err = HikingTrailClosuresError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(HikingTrailClosuresError)
