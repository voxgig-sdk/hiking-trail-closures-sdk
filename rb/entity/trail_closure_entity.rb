...
class TrailClosureEntity
  ...
  def make
    opts = @_entopts.dup
    TrailClosureEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = HikingTrailClosuresHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [TrailClosure, Hash] the current TrailClosure data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = HikingTrailClosuresHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of TrailClosure fields)
  def match_get
  ...
  #
  # @param reqmatch [TrailClosureListMatch, Hash, nil] match filter (any subset of
  #   TrailClosure fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<TrailClosure>, Array] the matching TrailClosure items; raises HikingTrailClosuresError on failure
