...
module HikingTrailClosuresUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = HikingTrailClosuresResponse.new({})
    ...
    if fetch_err
      ...
      response = HikingTrailClosuresResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = HikingTrailClosuresResponse.new(fetched)
    else
