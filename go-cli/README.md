# hiking-trail-closures-cli

boru-driven command-line client **and** interactive REPL for the HikingTrailClosures
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/hiking-trail-closures-cli)
make build

# 2. See usage (words, entities, env vars)
./hiking-trail-closures-cli --help

# 3. Provide credentials once, via the environment
export HIKING_TRAIL_CLOSURES_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./hiking-trail-closures-cli list trail_closure

# 5. Override the API base URL for a single call
HIKING_TRAIL_CLOSURES_BASE=https://api.example.com ./hiking-trail-closures-cli list trail_closure

# 6. No arguments -> interactive REPL
./hiking-trail-closures-cli
hiking-trail-closures> list trail_closure
hiking-trail-closures> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/hiking-trail-closures-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/hiking-trail-closures-cli list trail_closure
   ```
...
Run with no arguments to open a REPL (prompt `hiking-trail-closures>`). Each line is
evaluated as its own boru expression:

```text
$ ./hiking-trail-closures-cli
hiking-trail-closures> list trail_closure
hiking-trail-closures> /help
hiking-trail-closures> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `trail_closure`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/hiking-trail-closures-cli`. |
...
(`./hiking-trail-closures-cli <expr>`) and interactively (the REPL), and expressions compose the
