// Command HikingTrailClosures-cli is a CLI + REPL for the HikingTrailClosures SDK,
...
const prompt = "hiking-trail-closures"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "trail_closure"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      HIKING_TRAIL_CLOSURES_APIKEY (api key), HIKING_TRAIL_CLOSURES_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: HIKING_TRAIL_CLOSURES_APIKEY carries the API key and
	// HIKING_TRAIL_CLOSURES_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("HIKING_TRAIL_CLOSURES_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("HIKING_TRAIL_CLOSURES_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewHikingTrailClosuresSDK(opts)
