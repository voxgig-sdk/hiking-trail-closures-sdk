...
const __1 = require("..");
const SDK_NAME = 'HikingTrailClosuresSDK';
...
const TEST_SEED = { "entity": { "trail_closure": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+HikingTrailClosuresSDK\b/.test(code) ||
        /\bHikingTrailClosuresSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+HikingTrailClosuresSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/HikingTrailClosuresSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
