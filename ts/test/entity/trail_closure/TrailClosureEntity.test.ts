...
describe('TrailClosureEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'trail_closure.' + op, live)) return
    }
    ...
    const trail_closure_ref01_list = await trail_closure_ref01_ent.list(trail_closure_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['HIKING_TRAIL_CLOSURES_TEST_TRAIL_CLOSURE_ENTID']
