# hiking-trail-closures-mcp

[MCP](https://modelcontextprotocol.io) server exposing the HikingTrailClosures SDK as
two agent tools — `hiking-trail-closures_list` and `hiking-trail-closures_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/hiking-trail-closures-mcp)
make build

# 2. Provide credentials via the environment
export HIKING_TRAIL_CLOSURES_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user hiking-trail-closures \
  -- /absolute/path/to/hiking-trail-closures-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./hiking-trail-closures-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// hiking-trail-closures_list: first page of records
{ "entity": "trail_closure" }
{ "entity": "trail_closure", "query": { } }

// hiking-trail-closures_load: one record by id
{ "entity": "trail_closure", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/hiking-trail-closures-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user hiking-trail-closures \
     -- "$PWD"/dist/*/hiking-trail-closures-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `hiking-trail-closures_list` and `hiking-trail-closures_load` tools now appear
   in new sessions. Ask the agent to *"list trail_closure using hiking-trail-closures"*
   and it calls `hiking-trail-closures_list` with `{"entity":"trail_closure"}`.
...
```sh
export HIKING_TRAIL_CLOSURES_APIKEY=sk_live_xxx            # API key
export HIKING_TRAIL_CLOSURES_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `hiking-trail-closures_list` tool
...
### Call the `hiking-trail-closures_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `hiking-trail-closures_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `hiking-trail-closures_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `HIKING_TRAIL_CLOSURES_APIKEY` | API key sent with every request. |
| `HIKING_TRAIL_CLOSURES_BASE` | Optional override of the API base URL. |
...
```sh
./hiking-trail-closures-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"hiking-trail-closures_load","arguments":{"entity":"trail_closure","query":{"id":1}}}}'
```
