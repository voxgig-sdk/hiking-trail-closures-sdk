...
class TestTrailClosureDirect:

    def test_should_direct_list_trail_closure(self):
        setup = _trail_closure_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-trail_closure", "live" if setup["live"] else "unit")
        if _skip:
...
def _trail_closure_direct_setup(mockres):
    ...
    if live:
        ...
        client = HikingTrailClosuresSDK(merged_opts)
        return {
            "client": client,
            "calls": calls,
            "live": True,
