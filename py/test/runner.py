# HikingTrailClosures SDK test runner
...
class HikingTrailClosuresTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            HikingTrailClosuresTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = HikingTrailClosuresTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = HikingTrailClosuresTestRunner.getenv("HIKING_TRAIL_CLOSURES_TEST_LIVE")
        override = HikingTrailClosuresTestRunner.getenv("HIKING_TRAIL_CLOSURES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = HikingTrailClosuresTestRunner.getenv(key)
        ...
        explain = HikingTrailClosuresTestRunner.getenv("HIKING_TRAIL_CLOSURES_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if HikingTrailClosuresTestRunner._test_control is not None:
            return HikingTrailClosuresTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                HikingTrailClosuresTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            HikingTrailClosuresTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = HikingTrailClosuresTestRunner.load_test_control()
...
def load_env_local():
    HikingTrailClosuresTestRunner.load_env_local()


def env_override(m):
    return HikingTrailClosuresTestRunner.env_override(m)


def entity_data(v):
    return HikingTrailClosuresTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return HikingTrailClosuresTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return HikingTrailClosuresTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return HikingTrailClosuresTestRunner.load_test_control()


def live_client_options():
    return HikingTrailClosuresTestRunner.live_client_options()


def live_delay_ms():
    return HikingTrailClosuresTestRunner.live_delay_ms()
