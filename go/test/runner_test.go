...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("HIKINGTRAILCLOSURES_TEST_LIVE") == "TRUE" ||
		os.Getenv("HIKINGTRAILCLOSURES_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("HIKINGTRAILCLOSURES_TEST_EXPLAIN"); explain != "" {
