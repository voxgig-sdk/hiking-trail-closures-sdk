...
import (
	...
	omni "github.com/voxgig-sdk/hiking-trail-closures-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.HikingTrailClosuresError); is {
				out := map[string]any{
					"name":    "HikingTrailClosuresError",
...
func omniCtx(arg any, client *sdk.HikingTrailClosuresSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.HikingTrailClosuresSDK); is {
			client = live
