...
local TrailClosureEntity = {}
TrailClosureEntity.__index = TrailClosureEntity


function TrailClosureEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TrailClosureEntity)
  self._name = "trail_closure"
  self._client = client
...
function TrailClosureEntity:get_name()
  return self._name
end


function TrailClosureEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TrailClosureEntity.new(self._client, opts)
end
...
function TrailClosureEntity:mark_deleted()
  self._deleted = true
end


function TrailClosureEntity:deleted()
  return true == self._deleted
end


function TrailClosureEntity:data_set(args)
  if args ~= nil then
...
function TrailClosureEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TrailClosureEntity:match_set(args)
  if args ~= nil then
...
function TrailClosureEntity:match_get()
...
function TrailClosureEntity:stream(action, args, callopts)
...
---@param reqmatch TrailClosureListMatch
---@param ctrl? table
---@return TrailClosure[]
---@return string? err
function TrailClosureEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function TrailClosureEntity:_run_op(ctx, post_done)
  local utility = self._utility
